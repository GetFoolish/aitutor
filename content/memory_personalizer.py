...
class MemoryPersonalizer:
    ...
    def get_user_memories(self, user_id: str, limit: int = 50) -> List[UserMemory]:
        ...
        if self.memory_store:
            try:
                # Some stores (MongoDBMemoryStore) don't implement get_by_type
                if hasattr(self.memory_store, "get_by_type"):
                    ...
                    print("[memory_personalizer] Memory store has no get_by_type; using MongoDB fallback")
            except Exception as e:
            ...
            try:
                ...
                for result in results:
                    ...
                    if isinstance(mem, dict):
                        mem_type = mem.get("type", "personal")
                        mem_text = mem.get("text", "")
                        mem_importance = mem.get("importance", result.get("score", 0.5) if isinstance(result, dict) else 0.5)
                    else:
                        mem_type = getattr(mem, "type", "personal")
                        mem_text = getattr(mem, "text", str(mem))
                        mem_importance = getattr(mem, "importance", result.get("score", 0.5) if isinstance(result, dict) else 0.5)

                    memories.append(UserMemory(
                        type=mem_type.value if hasattr(mem_type, 'value') else str(mem_type),
                        text=mem_text,
