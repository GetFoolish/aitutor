...
def start_adaptive_assessment(subject: str, request: Request, force_mc: bool = False):
    ...
    try:
        ...
        # --- Fall through to DASH pool if JIT also failed ---
        if not first_q:
        ...
        if not first_q:
            # Last resort pool try: any grade (no grade range restriction)
            first_q = _run_with_timeout(
                dash_system.get_next_question_flexible,
                LEARNING_Q_LOOKUP_TIMEOUT_S,
                user_id,
...
def _load_question_perseus(question_id: str, mongo_db) -> Optional[dict]:
    ...
    AI-first: only loads from ai_generated_questions and content_pool.
    Khan Academy/scraped questions are no longer supported.
    """
