...
const AssessmentQuestion: React.FC<Props> = ({
  ...
  const sanitizedQuestion = useMemo(() => {
    ...
    try {
      ...
      if (typeof q.question.content === 'string') {
        let content = q.question.content;

        // 1. Fix literal &nbsp; and common AI artifacts
        content = content.replace(/&nbsp;/g, ' ').replace(/\\&nbsp;/g, ' ');

        // 2. Wrap standalone pseudocode blocks in triple backticks if they look like code
        // and aren't already wrapped. Pattern: multiple lines with indent or keywords
        const codeKeywords = [
          'FOR', 'IF', 'WHILE', 'FUNCTION', 'BEGIN', 'END', 'numbers =', 'sum =',
          'return', 'def ', 'class ', 'SET ', 'REPEAT', 'UNTIL', 'ELSE', 'THEN',
          'PRINT', 'INPUT', 'OUTPUT', 'DECLARE', 'VAR ', 'CONST ', 'array =', 'list ='
        ];
        ...
        for (const line of lines) {
          ...
          const looksLikeCode = codeKeywords.some(kw =>
          ...
          if (!inCodeBlock && (looksLikeCode || (isIndented && trimmed.length > 0))) {
            inCodeBlock = true;
            currentBlock.push(line);
          } else if (inCodeBlock && (looksLikeCode || isIndented || (trimmed.length === 0 && currentBlock.length > 0))) {
            currentBlock.push(line);
          } else {
            if (inCodeBlock) {
              // Wrap the detected block in triple backticks for Perseus rendering
              newLines.push('```\n' + currentBlock.join('\n').trimEnd() + '\n```');
              currentBlock = [];
              inCodeBlock = false;
            }
        ...
        // 3. Strip standalone image-reference sentences
        content = content
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setTimeout(() => setHasSubmitError(false), 4000);
      setIsAnswered(false);
    } finally {
      setIsSubmitting(false);
    }
  ...
  // Skill metadata extraction for breadcrumb
  const metadata = question?.dash_metadata || {};
  const skillNames = metadata.skill_names || [];
  const skillLabel = skillNames[0] || 'Assessment';

  return (
    <div
      ref={questionCardRef}
      className="framework-perseus relative flex w-full h-full min-h-0 items-stretch justify-center py-1 overflow-hidden"
      style={{ maxWidth: '100%' }}
    >
      {/* Card matching RendererComponent layout */}
      <Card className="relative flex h-full min-h-0 w-full flex-col border-[4px] md:border-[5px] border-black dark:border-white shadow-[2px_2px_0_0_rgba(0,0,0,1)] md:shadow-[2px_2px_0_0_rgba(0,0,0,1)] dark:shadow-[2px_2px_0_0_rgba(255,255,255,0.3)] md:dark:shadow-[2px_2px_0_0_rgba(255,255,255,0.3)] bg-[#FFFDF5] dark:bg-[#000000] transition-all duration-200 overflow-hidden" style={{ maxWidth: '100%' }}>
        {/* Progress bar at top — matching RendererComponent */}
        ...
        <CardHeader className={`${compactViewport ? "space-y-1.5 pt-5 px-3.5" : "space-y-2 pt-6 md:pt-7 px-4 md:px-6"} border-b-[3px] md:border-b-[4px] border-black dark:border-white bg-[#FFD93D] flex-shrink-0`}>
          <div className="flex items-start justify-between gap-3 md:gap-4 flex-wrap">
            <div className="space-y-1.5 flex-1">
              {/* Breadcrumb Navigation — assessment context */}
              <div className="flex items-center gap-2 flex-wrap text-xs md:text-sm font-bold text-black">
                ...
                <span className="uppercase tracking-wide">{skillLabel}</span>
              </div>
            </div>

            {/* Neo-Brutalist Progress Badge — matching RendererComponent */}
            <div className="flex items-center gap-2 md:gap-3">
              <div className="text-right hidden sm:block">
                ...
                <div className="text-xs md:text-sm font-black text-black">
                  Q <span className="text-[#FF6B6B]">{effectiveQuestionNumber || 1}</span>/{totalQuestions || '?'}
                </div>
              </div>
        ...
        <CardContent className={`${compactViewport ? "px-3 py-2.5" : "px-4 md:px-6 py-4 md:py-6"} bg-[#FFFDF5] dark:bg-[#000000] flex-1 min-h-0 overflow-hidden`}>
          <div
            ref={contentBlockRef}
            className={`relative w-full h-full min-h-0 overflow-x-hidden pr-1 ${resolvedContentZoom < 1 ? 'overflow-y-hidden' : 'overflow-y-auto'}`}
          >
            <div
              className={`${compactViewport ? "space-y-2.5" : "space-y-4 md:space-y-6"} h-full mx-auto`}
              style={{ ...contentZoomWrapperStyle, maxWidth: '100%' }}
            >
              ...
              <div
                ...
                className={`border-[3px] md:border-[4px] border-black dark:border-white bg-white dark:bg-neutral-800 text-black dark:text-white ${compactViewport ? 'p-3' : 'p-4 md:p-5 lg:p-6'} shadow-[2px_2px_0_0_rgba(0,0,0,1)] md:shadow-[2px_2px_0_0_rgba(0,0,0,1)] dark:shadow-[2px_2px_0_0_rgba(255,255,255,0.3)] md:dark:shadow-[2px_2px_0_0_rgba(255,255,255,0.3)] mx-auto`}
                style={{ overflowX: 'clip', overflowY: 'visible', maxHeight: 'none', flexShrink: 0, width: '100%', maxWidth: '800px' }}
              >
                ...
                {/* Progressive Hints — inside question card */}
                  ...
                  <div className="mt-4">
                      ...
                      <div className="mb-2">
                        {(question.hints || []).slice(0, hintsShown).map((hint: any, idx: number) => (
                          <div
                            ...
                            className="py-3 px-4 mb-3 border-[3px] border-black dark:border-white bg-[#FFF9C4] dark:bg-amber-900/40 shadow-[2px_2px_0_0_rgba(0,0,0,1)] dark:shadow-[2px_2px_0_0_rgba(255,255,255,0.3)] leading-relaxed break-words text-sm text-[#111827] dark:text-[#F9FAFB]"
                          >
                    ...
                    {hintsShown < (question.hints || []).length && (
                      <button
                        ...
                        className="py-2 px-4 text-xs font-black uppercase tracking-wide bg-[#FFD93D] dark:bg-[#FFD93D] text-black border-[3px] border-black dark:border-white cursor-pointer shadow-[2px_2px_0_0_rgba(0,0,0,1)] dark:shadow-[2px_2px_0_0_rgba(255,255,255,0.3)] mb-3 hover:bg-[#FFE066] hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-[1px_1px_0_0_rgba(0,0,0,1)] transition-all duration-100"
                      >
              ...
              {/* Inline feedback panel — matches RendererComponent style */}
              {isAnswered && showFeedback && keScore && (
                <div
                  ref={feedbackRef}
                  className="mt-4 border-[3px] md:border-[4px] border-black dark:border-white shadow-[4px_4px_0_0_rgba(0,0,0,1)] md:shadow-[6px_6px_0_0_rgba(0,0,0,1)] dark:shadow-[4px_4px_0_0_rgba(255,255,255,0.3)] animate-in slide-in-from-top-4 duration-300"
                >
                  <div className={`flex items-center gap-2 md:gap-3 px-5 md:px-6 py-3 md:py-4 ${keScore.correct ? 'bg-[#ADFF2F]' : 'bg-[#FF006E]'
                    }`}>
                    {keScore.correct ? (
                    ...
                    <span className={`text-base md:text-lg font-black uppercase tracking-tight ${keScore.correct ? 'text-black' : 'text-white'
                      }`}>
                      {keScore.correct ? 'Correct!' : 'Not quite!'}
                    </span>
                  </div>
                  ...
                  {!keScore.correct && question?.hints?.length > 0 && (
                    <div className="px-5 md:px-6 py-4 bg-[#FFF8E1] dark:bg-amber-900/30 border-t-[3px] border-black dark:border-white">
                      ...
                      <p className="text-sm leading-relaxed text-black dark:text-neutral-200">
                        {renderTextWithLatex(question.hints?.length ? (question.hints[question.hints.length - 1]?.content || question.hints[0]?.content || '') : '')}
                      </p>
                    </div>
                  )}
                </div>
        ...
        <CardFooter className={`${compactViewport ? "gap-2 px-3 py-2.5" : "gap-2 md:gap-3 px-4 md:px-6 pb-4 md:pb-5 pt-3 md:pt-4"} flex justify-between items-center border-t-[3px] md:border-t-[4px] border-black dark:border-white bg-white dark:bg-neutral-900 flex-shrink-0`}>
          {/* Hint button area (left side) */}
          {!isAnswered && question?.hints?.length > 0 && !brokenWidgetOnly ? (
            <Button
              type="button"
              variant="outline"
              size="sm"
              disabled={isSubmitting || hintsShown >= (question.hints || []).length}
              onClick={() => setHintsShown(h => h + 1)}
              className="transition-all duration-100 border-[2px] md:border-[3px] border-black dark:border-white bg-[#FFD93D] hover:bg-[#FFE066] text-black font-black uppercase tracking-wide shadow-[1px_1px_0_0_rgba(0,0,0,1)] md:shadow-[2px_2px_0_0_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 text-xs md:text-sm h-9 md:h-10 px-4 md:px-5"
            >
              Hint {hintsShown > 0 ? `(${hintsShown}/${(question.hints || []).length})` : ''}
            </Button>
          ) : <div />}
          ...
          <div className="flex gap-2 md:gap-3">
            ...
            {/* Next button — matches RendererComponent style */}
            <Button
              type="button"
              variant="outline"
              size="sm"
        ...
        {/* Auto-advance indicator bar — subtle, at very bottom */}
        {isAnswered && pendingCorrect !== null && (
          <div className="border-t-[2px] border-black dark:border-white" style={{ height: '4px', backgroundColor: '#eee', width: '100%', overflow: 'hidden' }}>
            <div style={{
              height: '100%',
      ...
      {/* Empty warning — outside card, floats above */}
      {emptyWarning && (
        <div
          id="empty-submit-warning"
          className="absolute bottom-20 left-4 right-4 py-3 px-4 border-[3px] border-black dark:border-white bg-[#FFF3E0] dark:bg-orange-900/40 shadow-[4px_4px_0_0_rgba(0,0,0,1)] dark:shadow-[4px_4px_0_0_rgba(255,255,255,0.3)] text-sm font-black text-[#E65100] dark:text-orange-300 uppercase tracking-wide text-center animate-bounce z-50"
