...
# Start the LiveKit Agent for video avatar (if USE_LIVEKIT is enabled)
if [[ "${USE_LIVEKIT:-false}" == "true" ]]; then
    echo "Starting LiveKit Agent (Video Avatar)... Logs -> logs/livekit_agent.log"
    (cd "$SCRIPT_DIR/services/LiveKitAgent" && "$PYTHON_BIN" agent.py dev) > "$SCRIPT_DIR/logs/livekit_agent.log" 2>&1 &
    pids+=($!)
else
    echo "ℹ️  LiveKit Agent skipped (set USE_LIVEKIT=true in .env to enable video avatar)"
fi
...
if [[ "${USE_LIVEKIT:-false}" == "true" ]]; then
    echo "  🎥 LiveKit Agent:      Running (Video Avatar enabled)"
fi
