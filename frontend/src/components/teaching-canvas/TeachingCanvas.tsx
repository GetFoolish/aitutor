...
> = (
  ...
  ref
    ...
    const redraw = useCallback(() => {
      ...
      // 2) Render static AI shapes (completed student drawings from previous turns)
      for (const { shape } of staticShapesRef.current) {
        animatorRef.current.renderShape(ctx, shape, 1);
      }

      // 3) Render AI-driven animations (both completed and active)
      animatorRef.current.renderAll(ctx);
    ...
    useEffect(() => {
      const canvas = canvasRef.current;
      if (!canvas) {
        console.warn("[TeachingCanvas] useEffect: canvas not available");
        return;
      }
      const ctx = canvas.getContext("2d");
      if (!ctx) {
        console.warn("[TeachingCanvas] useEffect: could not get 2d context");
        return;
      }

      const animator = animatorRef.current;
      console.log("[TeachingCanvas] Attaching animator to canvas context");
      animator.attach(ctx, () => {
      ...
      return () => {
        console.log("[TeachingCanvas] Detaching animator");
        animator.detach();
      };
    }, [redraw]);
    ...
    useImperativeHandle(ref, () => ({
      drawShapes(
        ...
        console.log("🎨 TeachingCanvas: drawShapes called", { count: shapes.length, options });
