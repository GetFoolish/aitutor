...
function FloatingControlPanel({
  ...
  useEffect(() => {
    ...
    function sendVideoFrame() {
      ...
      // Check if there is an active video source (Camera, Screen, or Canvas)
      // If all are off, we treat this as "paused" and stop sending frames efficiently
      const hasActiveSource = cameraEnabled || screenEnabled || isPaintActive;

      if (hasActiveSource && canvas.width + canvas.height > 0) {
      ...
      // We continue the loop even if paused, so it resumes immediately when sources are toggled on
  ...
  }, [connected, activeVideoStream, client, cameraEnabled, screenEnabled, isPaintActive]); // Removed refs from dependencies - they don't trigger re-renders
