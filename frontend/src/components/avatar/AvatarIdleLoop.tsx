 ...
 * 
 * Plays a 2-minute video loop when the avatar session is stopped or paused.
 * The video file should be placed at: frontend/public/videos/avatar-idle-loop.mp4
 * Falls back to animated gradient if video is not available.
 */
  ...
  return (
    <div className="w-full h-full relative">
      <video
        ...
        onLoadedMetadata={() => {
          // Video metadata loaded - verify audio track exists
          if (videoLoopRef.current) {
            const hasAudio = videoLoopRef.current.mozHasAudio || 
                            (videoLoopRef.current.audioTracks && videoLoopRef.current.audioTracks.length > 0);
            if (hasAudio) {
              console.debug('Avatar idle video: Audio track detected');
            }
          }
        }}
      >
